...
ts.set_token('d540bc13ad658cb002ecc5669c6236ea')
pro = ts.pro_api()
...
def get_quant_price(symbol: str = '000001.SZ'):
    ...
    传入symbol示例如下：
    “000858.SZ”，这是深市和创业板股票形式；
    “000001.SH”，这是沪市股票形式。
    
    前面是股票代码，后面大写字母标注所属市场。
    """
    ...
    if df.empty:
        return {"error": "未找到该股票数据，请传入合适的symbol，例如‘000858.SZ’或者‘000001.SH’"}
    ...
    result = f"这里展示了计算结果，对于该股票“{symbol}”的计算结果如下：“{result}”，一般认为股票现价小于极限位可以进行介入，而当股票现在的价格位于正常位和极限位之间时建议关注。"
    return result
